...
|---|---|
| Home — 4 envelope photos (headshot, grad, Betsy & Bernie, notepad) | `images/home/headshot.svg`, `grad-photo.svg`, `betsy-bernie.svg`, `notepad.svg` |
...
The About page's telegram and the CV page are now built from real text
(typed in the HTML), not images, so there's nothing to swap there — just
edit the words directly (see section 3 above).
...
```css
--color-bg: #F7F2EA;         /* page background — clean cream */
--color-charcoal: #33312D;   /* charcoal accent — buttons, links, nav */
--color-rose: #D8A79B;       /* light rose accent — eyebrows, help tab, dividers */
--font-display: 'Archivo', ...;    /* nav name, section subheads, body text */
--font-nav: 'Lora', ...;           /* the Home/About/Publications/... nav links */
--font-title: 'Caveat', ...;       /* every page's big handwritten title */
...
The palette is charcoal + light rose + cream, pulled from your color photo:
charcoal carries the "functional" parts of the site (buttons, links, the
current-page underline in the nav), and rose carries the "soft" decorative
touches (the small uppercase eyebrow labels, the help tab, the thin lines
under CV section headings). To retune either color everywhere at once,
just change `--color-charcoal` or `--color-rose` (and their `-dark`
partners, used for hover states).

Fonts: **Archivo** for the structural stuff (nav name, subheadings, body
text), **Lora** for the Home/About/Publications/etc. links in the top nav,
**Caveat** for both the big handwritten page titles (h1) and smaller
handwritten touches (photo captions, the "creative" label in the
Publications circle), **Newsreader** italic for one flowing accent line
per page (the `class="lede"` paragraphs, the help tab message, the
"Email:" line), and **Courier Prime** for document-style text (footer, CV
dates, the About page telegram).
...
ones, just edit the text inside the `.envelope__caption` span next to that
photo in `index.html` — no CSS needed.
